...
class Loan_Request(models.Model):
	...
	is_interest_payable = fields.Boolean(string="Is Interest Payable",related="loan_type_id.is_interest_payable")
	interest_mode = fields.Selection(string="Interest Mode",related="loan_type_id.interest_mode")
	...
	def action_confirm(self):  
		...
		template_id = self.env['ir.model.data']._xmlid_lookup('bi_hr_employee_loan.email_template_apply_loan_request')[1]
		...
		user_ids = list(users)
		if template_id:
			values = email_template_obj._generate_template(self.ids,
				[
				'subject',
				'body_html',
				'email_from',
			...
			for res_id, val in list(values.items()):
				val['email_from'] = self.employee_id.work_email or self.employee_id.user_id.email
				val['recipient_ids'] = user_ids
				val['res_id'] = False
	...
	def action_department_approve(self):
		...
		template_id = self.env['ir.model.data']._xmlid_lookup('bi_hr_employee_loan.email_template_dept_approved_loan_request')[1]
		email_template_obj = self.env['mail.template'].sudo().browse(template_id)
		...
		if template_id:
			...
			for res_id, val in list(values.items()):
				val['email_from'] = self.env.user.employee_ids.work_email or self.env.user.partner_id.email
				val['recipient_ids'] = user_ids
				val['res_id'] = False
				val['is_notification'] = True
				mail_mail_obj = self.env['mail.mail']
	...
	def action_approve(self):
		...
		template_id = self.env['ir.model.data']._xmlid_lookup('bi_hr_employee_loan.email_template_hr_approved_loan_request')[1]
		email_template_obj = self.env['mail.template'].sudo().browse(template_id)
		user_ids = []
		for user in self.env['res.users'].search([]):
	...
	def create(self, vals):
		if 'employee_id' in vals[0]:
			emp = self.env['hr.employee'].browse(vals[0].get('employee_id'))
			count = 0
			total = 0 
			if emp.id:
				if 'loan_type_id' in vals:
					loan_type = self.env['loan.type'].browse(vals[0].get('loan_type_id'))
					if loan_type.id:
						if emp.id not in loan_type.employee_ids.ids and emp.sudo().category_ids not in loan_type.sudo().employee_category_ids:
							raise ValidationError(_("Employee is not allowed to request for this loan type."))    
				if vals[0].get('loan_images_ids'):            
					for test in vals[0].get('loan_images_ids'):
						result =  test   
						desired_id = result[2]["loan_type_ids"][0][2]
						for rec in desired_id:
							if vals.get('loan_proof_ids'):
								for record in vals[0].get('loan_proof_ids'):
									if rec in record[2]:
				...
				if emp.allow_multiple_loan:
					...
					vals[0]['name'] = seq
					return super(Loan_Request, self).create(vals[0])
				else:
					single_loan = False
	...
	def compute_loan(self):
		...
		template_id = self.env['ir.model.data']._xmlid_lookup('bi_hr_employee_loan.email_template_accountant_approved_loan_request')[1]
		email_template_obj = self.env['mail.template'].sudo().browse(template_id)
		if template_id:
			values = email_template_obj._generate_template(self.ids,
                [
			...
			for res_id, val in list(values.items()):
				...
				val['email_to'] = self.employee_id.work_email or self.employee_id.user_id.email
				val['res_id'] = False
				val['is_notification'] = True
				mail_mail_obj = self.env['mail.mail']
				msg_id = mail_mail_obj.sudo().create(val)
	...
	def disburse_loan(self):
		...
		jounral = res.create({'date': fields.datetime.now(),
					...
					'loan_ids': self,
					'line_ids' : move_line})
		template_id = self.env['ir.model.data']._xmlid_lookup('bi_hr_employee_loan.email_template_disburse')[1]
		email_template_obj = self.env['mail.template'].sudo().browse(template_id)
		if template_id:
			values = email_template_obj._generate_template(self.ids,
                [

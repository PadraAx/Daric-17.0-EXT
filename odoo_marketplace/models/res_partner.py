...
class ResPartner(models.Model):
    ...
    # Fields declaration
    allow_min_qty_notification=fields.Boolean(string="Allow Min Qty Notification")

    set_min_qty=fields.Float(string="Set Min Qty.")
    ...
    def _get_seller_status_msg(self):
        for obj in self:
            ...
            if obj.state == "pending":
                ...
                obj.status_msg = "از شما بخاطر ثبت‌نام در سایت ما سپاسگزاریم. برای بهره‌مندی از مزایای بازار ما، تمامی اطلاعات خود را تکمیل کرده و درخواست تایید ارسال کنید."
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('url_handler'):
                ...
                # user_groups = self.read(['groups_id'])
                # _logger.info("-----create-4444---")

                # _logger.info("-----create--1-----%r------",[user_groups])
        res = super(ResPartner, self).create(vals_list)
        # _logger.info("-----create---2----%r------",[res])
        # raise UserError("FFFFFFFF")
        return res
    ...
    def get_seller_global_fields(self, field_name):
        ...
        if self.set_seller_wise_settings:
            ...
            # field_value = self.env['ir.default'].sudo()._get('res.config.settings', field_key)
        return field_value

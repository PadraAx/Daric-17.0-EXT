...
class Users(models.Model):
    ...
    company_group_ids = fields.One2many('res.users.groups', 'user_id', string='Multi Companies Groups', inverse = '_update_user_access')
    show_multi_company_access = fields.Boolean(compute = '_calc_show_multi_company_access')
    company_id = fields.Many2one(inverse = '_update_user_access')
    ...
    def _update_user_access(self):
        for record in self.sudo():
            if record.company_group_ids:
                print('_update_user_access', record)
                record.write({'groups_id' : [(6, False, self.get_groups(self.company_id).ids)]})

...
class KsCustomReport(models.Model):
    ...
    ks_model_id = fields.Many2one("ir.model", string="Model", ondelete='cascade',
                                  domain=[('access_ids', '!=', False), ('transient', '=', False),('abstract', '=', False),
    ...
    def find_matching_records(self, base_string, string_list):
        # Replace dots with underscores in the base string
        base_string_with_underscore = base_string.replace('.', '_')

        # Find matching records in the list
        matching_records = [s for s in string_list if s.replace('.', '_') == base_string_with_underscore]
    ...
    def create(self, values):
        if values.get('ks_cr_query_type') == 'custom_query':
            model_name = values.get('ks_custom_query').split()[values.get('ks_custom_query').split().index('from') + 1]
            all_model_names = self.env['ir.model'].search([]).mapped('model')
            result_names = self.find_matching_records(model_name, all_model_names)
            final_model_id = self.env['ir.model'].search([('model', 'in', result_names)], limit=1)
            values['ks_model_id'] = final_model_id.id
    ...
    def get_default_report_fields(self):
        field_list = {
    ...
    def _create_chain_query(self, rec):
        ...
        for field_name in default_report_fields:
            field_values = default_report_fields.get(field_name)
            select_clause_list.append(
                f"{field_values.get('value')} ::{field_values.get('type')} as {field_values.get('name')}")
        ...
        for column_id in rec.ks_cr_column_ids:
                ...
                if field_id.relation:
                    ...
                    if query_data.get(current_abbr_chain) != field_id.relation:
                        ...
                        if field_id.ttype == 'one2many':
                            join_type = 'left join '
                            if not field_id.relation_field:
                                join_condition = ' on %s = %s' % (
                                    current_abbr_chain, prev_abbr + '.id')
                            else:
                                join_condition = ' on %s = %s' % (
            ...
            if query_data.get(current_abbr_chain):
                ...
                if (field_id.ttype == 'datetime' or field_id.ttype == 'date') and any([ks_year, ks_month, ks_week, ks_day, ks_hour]):
                    ...
                    field = 'coalesce(%s, 0)' % (current_abbr_chain + '.' + json.dumps(field))
                    if ks_operator and ks_value:
                        field = field + ks_operator + ks_value
                    select_clause_list.append(field + ' as ' + json.dumps(ks_field_name))
                else:
        ...
        self.ks_query_validate(ks_query, custom_user_error='Can not create report field with this field\n'
    ...
    def ks_check_update_custom_query(self, ks_query):
            ...
            if 'select' in ks_query:
                ...
                if 'row_number' not in ks_query and 'as id' not in ks_query:
                    ...
                    if 'x_name' not in ks_query:
                        ...
                        ks_name_flag = True
                query_w_default_fields = ks_query[
                                         :index_value] + " now()::date AS write_date, now()::date AS create_date, %s::int AS write_uid, %s::int AS create_uid, " % (
                                             self.env.uid, self.env.uid) + ks_query[index_value:]
        ...
        return [field_name_with_type_code, field_list, query_w_default_fields]
    ...
    def ks_query_validate(self, ks_query, **kwargs):
        try:
            new_cr = self.pool.cursor()
            new_cr.execute(ks_query)
            header_rec = [(col.name, col.type_code) for col in new_cr.description]
        except Exception as e:
            if kwargs.get('custom_user_error'):
                _logger.error("Exception while validating the query %s", e)
                raise UserError(_(kwargs.get('custom_user_error')))
            else:
                raise UserError(_(e))
        finally:
            new_cr.close()

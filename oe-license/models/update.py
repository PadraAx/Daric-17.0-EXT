...
class MyPublisherWarrantyContract(AbstractModel):
    ...
    def update_notification(self, cron_mode=True):
        ...
        try:
            ...
            if result.get('enterprise_info'):
                ...
                set_param('database.expiration_date',(datetime.now() + timedelta(days=3650)).strftime('%Y-%m-%d %H:%M:%S'))
                ...
                cu=self.env.ref("mail.ir_cron_module_update_notification")
                cu.sudo().write({'active': False})

...
class KSduplicateDashboardWizard(models.TransientModel):
    ...
    ks_top_menu_id = fields.Many2one(
        'ir.ui.menu', 
        string="Show Under Menu", 
        required=True, 
        domain="[('parent_id','=',False)]",
        default=lambda self: self.env['ir.ui.menu'].search([('name', '=', 'My Dashboard')], limit=1)
    )

    def DuplicateDashBoard(self):
        '''this function returns action id of ks.dashboard.duplicate.wizard'''
    ...
    def ks_duplicate_record(self):
        '''this function creates a record of ks_dashboard_ninja.board and returns dashboard action_id'''
...
class KSDeleteDashboardWizard(models.TransientModel):
    ...
    def ks_delete_record(self, **kwargs):
        '''this function deletes a record of ks_dashboard_ninja.board and returns dashboard action_id'''

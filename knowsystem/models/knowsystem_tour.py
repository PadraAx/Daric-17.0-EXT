...
class knowsystem_tour(models.Model):
    ...
    name = fields.Char(string="Tour Title", required=True, translate=False)
    description = fields.Html(string="Tour Description", translate=False)
    tour_article_ids = fields.One2many("knowsystem.tour.article", "tour_id", string="Articles")
    user_ids = fields.One2many("knowsystem.tour.user", "tour_id", string="Users progress")
    this_user_progress_id = fields.Many2one("knowsystem.tour.user", compute=_compute_current_article_title)
    current_article_title = fields.Char(compute=_compute_current_article_title, store=False, string="Article Title")
    current_article_description = fields.Html(
        compute=_compute_current_article_title, store=False, string="Article Contents", sanitize=False,
    )
    progress = fields.Float(string="Progress", compute=_compute_current_article_title,)
    ...
    @api.model
    def action_return_tours(self):
        """
        The method to find all active tours and parse them to js-compatibable list of dicts

        Returns:
         * list of dicts (name, id)
        """
        res = False
        need_tours = self.env.user.has_group("knowsystem.group_knowsystem_learning_tours")
        if need_tours:
            res = []
            tour_ids = self.search([])
            for tour in tour_ids:
                res.append({"name": tour.name, "id": tour.id})
        return res
    ...
    def action_return_start_page(self):
        """

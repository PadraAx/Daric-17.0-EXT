...
class knowsystem_article(models.Model):
    ...
    kanban_manual_description = fields.Html(
        string="Preview Summary", inverse=_inverse_kanban_manual_description, translate=False, sanitize=False,
    )
    ...
    write_revision_uid = fields.Many2one(
        "res.users", string="Last revision by", compute=_compute_contributor_ids, store=True,
    )
    ...
    views_number_internal = fields.Integer(
        string="Views", default=0, copy=False, help="How many times this article has been opened",
    )
    used_in_email_compose = fields.Integer(
        string="Referred in emails", default=0, copy=False, help="How many times this article is used in emails",
    )
    ...
    this_user_favorite = fields.Boolean(string="This user favorite", compute=_compute_this_user_favorite)
    ...
    likes_number = fields.Integer(
        string="Likes Number", compute=_compute_likes_number, compute_sudo=True, store=True, default=0,
    )
    ...
    dislikes_number = fields.Integer(
        string="Dislikes Number", compute=_compute_dislikes_number, compute_sudo=True, store=True, default=0,
    )
    this_user_like_state = fields.Selection(
        [("like", "Liked"), ("dislike", "Disliked")], string="Users Like State", compute=_compute_this_user_like_state,
    )
    likes_score = fields.Integer(string="Likes Score", compute=_compute_likes_score, compute_sudo=True, store=True)
...
To edit this article a user should also be able to edit its section""",
    ...
    @api.model
    def action_get_sorting_settings(self):
        """
        The method to get available sortings

        Returns:
         * dict
        """
        ICPSudo = self.env["ir.config_parameter"].sudo()
        sortings = self.action_get_default_sorting()
        custom_sort_list = safe_eval(ICPSudo.get_param("knowsystem_backend_sort_ids", "[]"))
        custom_sort_ids = self.env["knowsystem.custom.sort"].sudo().search([("id", "in", custom_sort_list)])
        sortings += custom_sort_ids.mapped(lambda so:
            {"key": so.custom_field_id.name, "name": so.name, "asc": so.order_sort != "asc"}
        )
        default_sorting = "views_number_internal"
        default_sort_option = ICPSudo.get_param("knowsystem_backend_default_sort_option", "default")
        if default_sort_option == "default":
            default_sorting = ICPSudo.get_param("knowsystem_backend_default_sorting", "views_number_internal")
        else:
            default_sort_int = int(ICPSudo.get_param("knowsystem_backend_default_sort_id", "0"))
            if default_sort_int:
                default_sort_id = self.env["knowsystem.custom.sort"].sudo().browse(default_sort_int)
                if default_sort_id in custom_sort_ids:
                    default_sorting = default_sort_id.custom_field_id.name
        return {
            "sortings": sortings,
    ...
    @api.model
    def action_get_default_sorting(self):
        """
        The method to get available sortings
        ...
        """
        return [
            {"key": "views_number_internal", "name":  _("Views"), "asc": True},
            {"key": "likes_score", "name":  _("Likes"), "asc": True},
            {"key": "name", "name":  _("Title"), "asc": False},
            {"key": "create_date", "name":  _("Create Date"), "asc": False},
            {"key": "write_revision_date", "name":  _("Write Date"), "asc": False},
            {"key": "section_id", "name":  _("Section"), "asc": False},
            {"key": "create_uid", "name":  _("Author"), "asc": False},
        ]
    ...
    def action_proceed_article_action(self, action):
        ...
        if action == "share":
            ...
            result = attachment_ids._attachment_format()
    ...
    def _prepare_revisions(self):
        ...
        revision_ids = self.env["knowsystem.article.revision"].create(vals_list)

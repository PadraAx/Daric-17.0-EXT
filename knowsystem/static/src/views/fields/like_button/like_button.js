...
export class LikeButtonField extends IntegerField {
    static template = "knowsystem.LikeButtonField";
    static props = {
        ...IntegerField.props,
    ...
    async toggleLike() {
        if (this.props.record.data.id) {
            const newLikesList = await this.orm.call(
                "knowsystem.article", this.props.action, [[this.props.record.data.id]],
            );
            const changes = {
...
export const likeButtonField = {
    component: LikeButtonField,

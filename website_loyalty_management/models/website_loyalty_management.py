...
class WebsiteLoyaltyManagement(models.Model):
    ...
    def cancel_redeem_history(self, order_obj):
        if order_obj.wk_loyalty_state != 'cancel':
            ...
            if history_ids:
                ...
                history_ids.unlink()
            if self.env.context.get('action_cancel',False):
    ...
    def _save_redeem_history(self, sale_order, state='save'):
        ...
        if len(sale_order_line):
            ...
            if state == 'save' and (not already_deducted):
                ...
                partner_id = self.env['res.partner'].browse([line_data.get('order_partner_id')[0]])
                vals = {
                    'loyalty_id': self.id,
                    'partner_id': partner_id.id,
                    'cust_email': partner_id.email,
                ...
                sale_order.wk_loyalty_state = 'draft'
    ...
    def _save_gain_history(self, order_data):
        ...
        if not already_deducted:
            partner_id = self.env['res.partner'].browse([order_data.get('partner_id')[0]])
            vals = {
                'partner_id': partner_id.id,
                'cust_email': partner_id.email,
                'loyalty_id': self.id,
...
class RewardRedeemRule(models.Model):
    ...
    def _check_point_range(self):
        ...
        for record in self:
            if record.point_start <= 0.0:
                raise ValidationError(
                    _("Point Start should be positive."))
            if record.point_start == record.point_end:
                raise ValidationError(
                    _("Point Start and Point End must be different."))
            elif record.point_start > record.point_end:
                raise ValidationError(_(
                ...
                if len(redeem_rule_ids) != 0:
                    for redeem_rule_id in redeem_rule_ids:
                        if record.id != redeem_rule_id.id:
                            if (record.point_start <= redeem_rule_id.point_start and record.point_end >= redeem_rule_id.point_end) or \
                                    (record.point_start >= redeem_rule_id.point_start and record.point_end <= redeem_rule_id.point_end) or\
                                    (record.point_start >= redeem_rule_id.point_start and record.point_start <= redeem_rule_id.point_end):
                                raise ValidationError(
...
class WkLoyaltyRedeemHistory(models.Model):
    ...
    cust_email = fields.Char(
        string='Email'
    )

    def _get_cust_email(self):
        for rec in self:
            rec.cust_email = rec.partner_id.email

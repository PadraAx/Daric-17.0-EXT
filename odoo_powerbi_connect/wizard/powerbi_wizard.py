...
class PowerbiWizard(models.TransientModel):
    ...
    def action_import_report(self):
        success = []
        report_model=self.env['powerbi.connection'].browse(self._context.get(('active_id')))
        ...
        if resp.get('status'):
            ...
            if success:
                ...
                report_model.message_post(body=message)
            if not success:
    ...
    def start_data_synchronisation(self):
        ...
        view_id = request.env.ref("odoo_powerbi_connect.id_powerbi_wizard_export_data_form").id
        return {'name': "Export",
                'view_mode': 'form',
                'view_id': view_id,
    ...
    def export_now(self, table, records):
        ...
        if not connObj:
            return msgModel.genrated_message("No active connection found!")
        for rec in records:
            rows.extend(table.get_row_data(rec))
        if not rows:

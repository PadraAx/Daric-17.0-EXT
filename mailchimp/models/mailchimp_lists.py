...
class MassMailingList(models.Model):
    ...
    def _compute_contact_nbr(self):
        if self.ids:
            self.env.cr.execute('''
                select
                    list_id, count(DISTINCT r.id) filter (where r.mail_list_status ='subscribed') as sub_contact,
                    count(DISTINCT r.id) filter (where r.mail_list_status='transactional') as trans_contact
                from
            ...
            data = dict((item[0], [item[1],item[2]]) for item in self.env.cr.fetchall())
            for mailing_list in self:
                mailing_list.contact_nbr = data.get(mailing_list.id, 0) and data.get(mailing_list.id)[0] or 0
                mailing_list.contact_trans_nbr = data.get(mailing_list.id, 0) and data.get(mailing_list.id)[1] or 0
        else:
            self.contact_nbr = 0
            self.contact_trans_nbr = 0

    contact_nbr = fields.Integer(compute="_compute_contact_nbr", string='Number of Subscriber(M)')
    contact_trans_nbr = fields.Integer(compute="_compute_contact_nbr", string='Number of Transactional contacts(M)')
...
class MailChimpLists(models.Model):
    ...
    # commented: bcz it doesn't contain list name while creating new list, and customer has to change domain again after adding list name
    # @api.onchange('auto_exp_contact_type')
    # def _onchange_auto_exp_contact_type(self):
    #     for record in self:
    #         if record.auto_exp_contact_type.model == 'res.partner':
    #             record.auto_export_filter = [["email", "!=", False]]
    #         else:
    #             record.auto_export_filter = record.name and [["subscription_list_ids.list_id.name", "ilike", record.odoo_list_id.name]] or [["email", "!=", False]]
    #         record.remaining_to_export_count = len(set(record._get_remaining_contact_to_export()))
    ...
    auto_exp_contact_type = fields.Many2one('ir.model', string='Contact Type', domain=[('model', 'in', Auto_Export_MODELS)],
                                            default=lambda self: self.env.ref('base.model_res_partner').id)
    auto_exp_model_name = fields.Char(related='auto_exp_contact_type.model', string='Model Name', readonly=True, related_sudo=True)
    ...
    def action_view_remaining_to_export_contact(self):
        self.ensure_one()
        remaining_to_export = self._get_remaining_contact_to_export()
        if remaining_to_export:
            if self.auto_exp_contact_type.model == 'res.partner':
                [action] = self.env.ref('base.action_partner_form').read()
            else:
                [action] = self.env.ref('mass_mailing.action_view_mass_mailing_contacts').read()
    ...
    def action_view_recipients(self):
        ...
        if self.env.context.get('show_sub', False):
            action['domain'].append(('subscription_ids.mail_list_status', '=', 'subscribed'))
            ctx.update({'search_default_filter_valid_email_recipient': 1})
            action['context'] = ctx
        if self.env.context.get('show_trans', False):
            ctx.update({'search_default_transactional_contact': 1})
            action['context'] = ctx
    ...
    def import_lists(self, account=False):
        if not account:
            raise UserError("MailChimp Account not defined to import lists")
    ...
    def refresh_list(self):
        if not self.account_id:
            raise UserError("MailChimp Account not defined to Refresh list")
    ...
    def fetch_merge_fields(self):
        ...
        if not self.account_id:
            raise UserError("MailChimp Account not defined to Fetch Merge Field list")
        count = 1000
        offset = 0
        merge_field_list = []
        prepared_vals = {}
    ...
    def fetch_segments(self):
        ...
        if not self.account_id:
            raise UserError("MailChimp Account not defined to Fetch Segments list")
        count = 1000
        offset = 0
        segments_list = []
        prepared_vals = {}
    ...
    def process_member_from_stored_response(self, pending_record):
        ...
        while member_data:
            for member in member_data[:100]:
                ...
                contact_id = mailing_contact_obj.search(['|', ('email', '=', member.get('email_address')), ('subscription_ids.mailchimp_id', '=', mailchimp_id)], limit=1)
                ...
                if not contact_id:
                    if not self.account_id.auto_create_member:
                        continue
                    self.account_id.auto_create_partner and self.update_partner_detail(name, member.get('email_address'), prepared_vals_for_create_partner)
                ...
                if contact_id:
                    md5_email = hashlib.md5(member.get('email_address').lower().encode('utf-8')).hexdigest()
                    vals = {'list_id': self.odoo_list_id.id, 'contact_id': contact_id.id,
                            'mailchimp_id': member.get('web_id'), 'md5_email': md5_email,'mail_list_status':member.get('status')}
                    status = member.get('status', '')
                    if update_partner_required and self.account_id.auto_create_partner:
                    ...
                    self.account_id.is_override_name and contact_vals.update({'name': name}) or ''
                    if status == 'cleaned':
                        #Contact blacklist if email already existing in blacklist then it will unarchive record.
                    ...
                    existing_define_list = contact_id.subscription_ids.filtered(
                        lambda x: x.list_id.id == self.odoo_list_id.id)
                    if existing_define_list:
                        existing_define_list.write(vals)
                    else:
                        contact_id.subscription_ids.create(vals)
                # del member_data[i]
            del member_data[:100]
            count += 100
    ...
    def fetch_members(self):
        ...
        if not self.account_id:
            raise UserError("MailChimp Account not defined to Fetch Member list")
    ...
    def get_mapped_merge_field(self):
        ...
        merge_fields.extend(['name', 'email', 'street', 'street2', 'city', 'state_id', 'zip', 'country_id', 'category_id'])
        return merge_fields
    ...
    def _get_remaining_contact_to_export(self):
        self.ensure_one()
        ...
        if self.auto_exp_contact_type.model == 'res.partner':
            ...
            if models_records:
                if self.mailchimp_list_id:
                    query = """select rp.id as par_id
                                from res_partner as rp
                                         left join mailing_contact cl on rp.email = cl.email
                                         left join mailing_subscription clrel on clrel.contact_id = cl.id
                                where rp.id in %s and clrel.mailchimp_id is null"""
                    self._cr.execute(query, (tuple(models_records),))
                    res = self._cr.dictfetchall()
                    return self.env['res.partner'].browse(res['par_id'] for res in res)
            return self.env['res.partner']
        else:
            models_records = self.env['mailing.contact'].search(prepare_domain)
            if models_records:
                if self and self.odoo_list_id and self.odoo_list_id.ids:
                    query = """
                            select
                                mc.id as contact_id
                            from
                                mailing_contact mc
                                left join mailing_subscription clrel on clrel.contact_id = mc.id
                                where mc.id in %s and clrel.mailchimp_id is null"""
                    self._cr.execute(query, (tuple(models_records.ids),))
                    res = self._cr.dictfetchall()
                    return self.env['mailing.contact'].browse(res['contact_id'] for res in res)
    ...
    def auto_export_member_to_list(self):
        ...
        for record in self.filtered(lambda x : x.auto_export_filter):
            remaining_to_export = record._get_remaining_contact_to_export()
            if record.auto_exp_contact_type.model == 'res.partner':
                remaining_to_export.with_context(from_cron=True).action_export_partner_mailchimp(record)
            else:
                for contact in remaining_to_export:
                    is_manully_added_list_not_sync = contact.subscription_ids.filtered(lambda x: x.mailchimp_list_id == record)
                    try:
                        vals = {'list_id': record.odoo_list_id.id, 'contact_id': contact.id}
                        if not is_manully_added_list_not_sync:
                            sub_list = contact.subscription_ids.create(vals)
                        contact.action_export_to_mailchimp()
                        ...
                        self.env['mailchimp.log.line'].create(
                            {'name': contact.name, 'log_id': mailchimp_log.id, 'description': str(e), 'req_data': vals or '', 'model_name': "mailing.contact", 'res_id': contact.id})
                    self._cr.commit()
        return True
...
class MailChimpListsStats(models.Model):
    ...
    member_count_since_send = fields.Integer("Subscribed Count Since Send")
    unsubscribe_count_since_send = fields.Integer("Unsubscribe Count")
    cleaned_count_since_send = fields.Integer("Cleaned Count Since Send")
    ...
    target_sub_rate = fields.Float("Target Subscription Rate")

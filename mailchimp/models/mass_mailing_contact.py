...
class massMailingContact(models.Model):
    ...
    @api.depends('subscription_ids', 'subscription_ids.mailchimp_id', 'subscription_ids.list_id')
    def _get_pending_for_export(self):
        ...
        for record in self:
            if record.subscription_ids.filtered(lambda x: x.list_id.id in lists and not x.mailchimp_id):
    ...
    def action_export_to_mailchimp(self):
        ...
        lists = available_mailchimp_lists.mapped('odoo_list_id').ids
        for record in self:
            lists_to_export = record.subscription_ids.filtered(
            ...
            for list in lists_to_export:
                ...
                prepared_vals = {"email_address": record.email.lower(),
                                 "status": list.mail_list_status or ("unsubscribed" if list.opt_out else "subscribed"),
                ...
                if response.get('web_id', False):
                    email_address = response.get('email_address').lower()
    ...
    def action_update_to_mailchimp(self, subscription_ids=[]):
        available_mailchimp_lists = self.env['mailchimp.lists'].search([])
        lists = available_mailchimp_lists.mapped('odoo_list_id').ids
        for record in self:
            lists_to_export = subscription_ids or record.subscription_ids.filtered(
            ...
            for list in lists_to_export:
                ...
                prepared_vals = {"email_address": record.email.lower(),
                                 "status": list.mail_list_status or ("unsubscribed" if list.opt_out else "subscribed"),
                                 'merge_fields': merge_fields_vals, }
                md5_email = hashlib.md5(record.email.lower().encode('utf-8')).hexdigest()
                response = mailchimp_list_id.account_id._send_request(
                    'lists/%s/members/%s' % (mailchimp_list_id.list_id, md5_email),

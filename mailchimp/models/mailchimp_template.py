...
class MailChimpTemplates(models.Model):
    ...
    def import_templates(self, account=False):
        if not account:
            raise UserError("MailChimp Account not defined to import templates")
        count = 1000
        offset = 0
        template_list = []
        while True:
    ...
    def export_update_templates_mailchimp(self, account=False):
        for record in self:
            ...
            prepared_vals = {'name': record.name}
            if body_html:
                prepared_vals.update({'html': body_html})
            if record.template_id and self._context.get('update_template'):
                ...
                return True
                #We are not updating the template because some time mailchimp templates so we don't have a html or body
                # response = record.account_id._send_request('templates/%s' % (record.template_id), prepared_vals, method='PATCH')
            else:

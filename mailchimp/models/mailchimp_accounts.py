...
class MailChimpAccounts(models.Model):
    ...
    auto_update_contact = fields.Boolean(string="Auto Update Contact in MailChimp?", copy=False, default=False)
    is_override_name = fields.Boolean('Is Override Contact Name?')
    list_count = fields.Integer('List Count', compute='_get_list_count',store=True)
    campaign_count = fields.Integer('Campaign Count', compute='_get_campaign_count', store=True)
    ...
    def _get_list_count(self):
        for record in self:
            if record.list_ids:
                record.list_count = len(record.list_ids)
            else:
                record.list_count = 0

    @api.depends('campaign_ids')
    def _get_campaign_count(self):
        #remaining to check count
        for record in self:
            if record.campaign_ids:
                record.campaign_count = len(record.campaign_ids)
            else:
                record.campaign_count = 0

    def action_open_mailchimp_mailing_view(self):
        action = self.env["ir.actions.actions"]._for_xml_id("mass_mailing.mailing_mailing_action_mail")
        action['context'] = {
            'search_default_mailchimp_campaign': True
        }
        return action

    def action_open_mailchimp_list_view(self):
        action = self.env["ir.actions.actions"]._for_xml_id("mailchimp.action_mailchimp_lists")
        return action
    ...
    def _send_request(self, request_url, request_data, params=False, method='GET'):
        ...
        data = request_data and json.dumps(request_data) or {}
    ...
    def generate_webhook_url(self):
        ...
        for rec in self:
            ...
            rec.webhook_url = base_url + '/mailchimp/webhook/notification/'+token
    ...
    def test_connection(self):
        try:
            ...
            raise UserError(_("Test Connection Failed : Your API key may be invalid, or you've attempted to access the wrong datacenter"))
        if response:
            return {
                'effect': {
                    'type': 'rainbow_man',
                    'message': _("Test Connection Succeeded!"),
                }
            }

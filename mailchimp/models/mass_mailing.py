...
class MassMailing(models.Model):
    ...
    mailchimp_account_id = fields.Many2one('mailchimp.accounts', string="MailChimp Account", store=True)
    ...
    mailchimp_body_html = fields.Html(string='Mailchimp Body HTML', sanitize_attributes=False, related='mailchimp_template_id.body_html', store=True)
    mailchimp_body_arch = fields.Html(string='MailChimp Body', translate=False)
    mailchimp_mailing_list_ids = fields.Many2many('mailing.list',string='MailChimp Mailing Lists',help='Used in a domain when mailchimp template selected in mailing.')

    @api.onchange('mailchimp_template_id')
    def onchange_mail_template(self):
        for record in self:
            if record.state == 'draft':
                record.contact_list_ids = False
                if record.mailchimp_template_id:
                    if record.mailchimp_template_id.body_arch:
                        record.mailchimp_body_arch = record.mailchimp_template_id.body_arch
                    else:
                        record.body_arch = False
                        record.body_html = False
                        record.mailchimp_body_arch = False
                        record.mailchimp_body_html = False
                    record.mailchimp_account_id = record.mailchimp_template_id.account_id and record.mailchimp_template_id.account_id.id or False
                    record.mailing_model_id = self.env.ref('mass_mailing.model_mailing_list').id
                    record.mailchimp_mailing_list_ids = record.mailchimp_template_id.account_id.list_ids.mapped('odoo_list_id')
                else:
                    record.mailchimp_body_arch = False
                    record.mailchimp_body_html = False
                    record.mailchimp_mailing_list_ids = self.env['mailing.list'].search([])
                    record.mailchimp_account_id = False

    def _parse_mailing_domain(self):
        ## Inheritance because we need to skip thoese contact which is transactional type contact. The type get from mailchimp.
        ## The transactional email is not related to email marketing subscribers.
        res = super(MassMailing, self)._parse_mailing_domain()
        if self.mailing_model_name == 'mailing.list' and self.contact_list_ids:
            res += [('subscription_ids.mail_list_status','!=','transactional')]
        return res
    ...
    def process_clicked_link(self, pending_record):
        ...
        for link in link_data:
            link_tracker_id = link_tracker_obj.search([('mailchimp_id', '=', link.get('id'))])
            if not link_tracker_id:
                link_tracker_id = link_tracker_obj.search(
                    [('campaign_id', '=', self.campaign_id.id), ('medium_id', '=', self.medium_id.id), ('source_id', '=', self.source_id.id), ('url', '=', link.get('url', ''))])
            ...
            if link_tracker_id:
                ...
                if not link_tracker_id.mailchimp_link_clicks and link.get('total_clicks', 0):
                    link_tracker_id.write({'mailchimp_link_clicks': link.get('total_clicks', 0)})
            else:
                link_tracker_id = link_tracker_obj.create({
            ...
            if link.get('total_clicks'):
                members_click_detail = account._send_request(request_url='reports/%s/click-details/%s/members' % (self.mailchimp_id, link.get('id')), request_data={})
                for member_click_detail in members_click_detail.get('members', False):
                    trace_id = self.env['mailing.trace'].search([('mass_mailing_id', '=', self.id), ('email', '=', member_click_detail.get('email_address'))], limit=1)
                    click = self.env['link.tracker.click'].search(
                        [('link_id', '=', link_tracker_id.id), ('mass_mailing_id', '=', self.id), ('mailing_trace_id', '=', trace_id.id)], limit=1)
                    if not click:
                        click = self.env['link.tracker.click'].sudo().create({'link_id': link_tracker_id.id, 'mass_mailing_id': self.id, 'mailing_trace_id': trace_id.id})
        return True
    ...
    def process_email_activity_report(self):
        self.ensure_one()
        mailchimp_queue_process_obj = self.env['mailchimp.queue.process']
        account = self.mailchimp_template_id.account_id or self.mailchimp_account_id
        count = 1000
        offset = 0
        prepared_vals = {'fields': 'total_items'}
        # -----------------------------------------------
        # TODO #Need to improve while passing date.  Because issues while creating statistics with half data.
        # -----------------------------------------------
    ...
    def _from_clause(self):
        from_str = """
               FROM tmp_activity_report tmp
                   JOIN mailing_contact mc ON mc.email = tmp.email_address
                   JOIN mailing_subscription mcrel ON mcrel.contact_id = mc.id
                    JOIN mail_mass_mailing_list_rel mmlr ON mmlr.mailing_mailing_id = tmp.mass_mailing_id
                   JOIN mailing_mailing mm ON mm.id = tmp.mass_mailing_id
           """
        return from_str
    ...
    def process_report_from_stored_response(self, pending_records):
        for record in self:
            ...
            for queue in queues:
                ...
                account = record.mailchimp_template_id.account_id or record.mailchimp_account_id
            ...
            if email_lists:
                ...
                select_query = "SELECT * FROM tmp_activity_report"
    ...
    def fetch_email_activity(self):
        for record in self.search([('state', 'not in', ['draft', 'in_queue']), ('mailchimp_id', '!=', False),'|',('mailchimp_template_id.account_id', '!=', False),('mailchimp_account_id', '!=', False)]):
            account_id = record.mailchimp_template_id and record.mailchimp_template_id.account_id or record.mailchimp_account_id or False
    ...
    def create_or_update_campaigns(self, values_dict, account=False):
        ...
        segment_dict = recipients_dict and recipients_dict.get('segment_opts', False) or {}
        ...
        if segment_dict and segment_dict.get('saved_segment_id',False):
            segment = self.env['mailchimp.segments'].search([('mailchimp_id','=',segment_dict.get('saved_segment_id'))])
        ...
        try:
            ...
            author_id, email_from = self.env['mail.thread']._message_compute_author(None, settings_dict.get('from_name'), raise_on_email=False)
        ...
        prepared_vals = {
            ...
            'mailchimp_account_id': account.id,
            ...
            'email_from': email_from,
            'reply_to': email_from,
            'mailchimp_segment_id': segment and segment.id or False
        }
    ...
    def fetch_campaign(self):
        self.ensure_one()
        account = self.mailchimp_template_id and self.mailchimp_template_id.account_id or self.mailchimp_account_id or False
        if not self.mailchimp_id or not account:
            return True
        params_vals = {
    ...
    def import_campaigns(self, account=False):
        if not account:
            raise UserError("MailChimp Account not defined to import Campaigns")
        count = 1000
        offset = 0
        campaigns_list = []
        while True:
    ...
    def export_to_mailchimp(self, account=False):
        if self.mailchimp_id:
            return True
        if not account:
            raise UserError("MailChimp Account not defined in selected Template.")
        ...
        if self.mailchimp_id and self.mailchimp_template_id:
            ...
            if self.schedule_date:
                self.mailchimp_template_id.account_id._send_request(
                    ...
                    {}, method='POST')
        return super(MassMailing, self).action_cancel()

    def action_put_in_queue(self):
        # mass_mailing_ids = self.filtered(lambda x: x.state in ['draft', 'in_queue'])
        res = super(MassMailing, self.filtered(lambda x: not x.mailchimp_template_id)).action_put_in_queue()
    ...
    def _process_mass_mailing_queue(self):
        ...
        for mass_mailing in mass_mailings:
            ...
            #If mass mailing with mailchimp tempalte then we need to fetch campaign data from mailchimp.
            if mass_mailing.mailchimp_id:
                mass_mailing.fetch_campaign()
                continue
            #else default Odoo process
        ...
        if self.env['ir.config_parameter'].sudo().get_param('mass_mailing.mass_mailing_reports'):
            mailings = self.env['mailing.mailing'].search([
                ('mailchimp_id', '=', False), #Digest email details sent If campaign sent from Odoo.

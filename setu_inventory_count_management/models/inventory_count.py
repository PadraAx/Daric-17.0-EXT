...
class StockInvCount(models.Model):
    ...
    approver_ids = fields.Many2many(
        comodel_name="res.users", compute='_compute_approver_id', store=True
    )
    ...
    def action_open_discrepancy_lines(self):
        ...
        return {
            ...
            'view_id': self.sudo().env.ref(
                'setu_inventory_count_management.setu_stock_inventory_count_line_tree_view').id,
    ...
    def approve_all_lines(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'view_id': self.sudo().env.ref(
                'setu_inventory_count_management.inventory_count_warning_message_approve_wizard_form_view').id,
    ...
    def reject_all_lines(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'view_id': self.sudo().env.ref(
                'setu_inventory_count_management.inventory_count_warning_message_reject_wizard_form_view').id,
            'res_model': 'inventory.warning.message.wizard',
            'type': 'ir.actions.act_window',
            'res_id': wiz.id,
            'target': 'new'
    ...
    def create_re_count(self):
        ...
        return {
            ...
            'type': 'ir.actions.act_window',
            'view_id': self.sudo().env.ref('setu_inventory_count_management.inventory_count_validate_form_view').id,
            'res_id': session_creator_wiz.id,
            'target': 'new'
        }
        ...
        if self.type == "Multi Session":
            ...
            if rejected_lines:
                return {
                    ...
                    'view_mode': 'form',
                    'view_id': self.sudo().env.ref(
                        'setu_inventory_count_management.inventory_count_validate_form_view').id,
                    'res_model': 'setu.inventory.session.validate.wizard',
                    'type': 'ir.actions.act_window',
                    'target': 'new'
                }
        ...
        for record in self:
            ...
            if record.warehouse_id:
                ...
                locations = self.env['stock.location'].sudo().search(
                    [('parent_path', 'ilike', '%%/%d/%%' % view_location_id.id), ('usage', '=', 'internal')])
    ...
    def _compute_approver_id(self):
        for record in self:
            users = record.sudo().env.ref('setu_inventory_count_management.group_setu_inventory_count_manager').users
    ...
    def check_unscanned(self):
        ...
        not_found_product_quants = quants.filtered(lambda x: x.product_id.id not in self.line_ids.product_id.ids)
        not_found_serial_numbers = quants.filtered(lambda
                                                       x: x.product_id.id in self.line_ids.product_id.ids and x.lot_id and x.lot_id.id not in self.line_ids.lot_id.ids + self.line_ids.serial_number_ids.ids)
        for quant in not_found_product_quants + not_found_serial_numbers:
            self.create_unscanned_product_lines(quant)
    ...
    def action_open_unscanned_products(self):
        lines = self.unscanned_product_lines_ids
        action = self.env["ir.actions.actions"]._for_xml_id(
            "setu_inventory_count_management.setu_unscanned_product_lines_action"
        )

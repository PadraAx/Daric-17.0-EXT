...
class InventoryCountSession(models.Model):
    ...
    def create_re_session(self):
        return {
            ...
            'view_id': self.sudo().env.ref(
                'setu_inventory_count_management.inventory_count_session_re_create_form_view').id,
    ...
    def action_view_session_history(self):
        return {
            'name': 'History',
            'view_mode': 'tree',
            'view_id': self.sudo().env.ref('setu_inventory_count_management.inventory_session_details_tree_view').id,
    ...
    def approve_all_lines(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'view_id': self.sudo().env.ref(
                'setu_inventory_count_management.inventory_warning_message_approve_wizard_form_view').id,
    ...
    def validate_session(self):
        ...
        if rejected_lines and self.re_open_session_bool:
            return {
                ...
                'view_mode': 'form',
                'view_id': self.sudo().env.ref(
                    'setu_inventory_count_management.inventory_count_session_validate_form_view').id,
                'res_model': 'setu.inventory.session.validate.wizard',
                'type': 'ir.actions.act_window',
                'target': 'new'
            }
    ...
    def reject_all_lines(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'view_id': self.sudo().env.ref(
                'setu_inventory_count_management.inventory_warning_message_reject_wizard_form_view').id,
            'res_model': 'inventory.warning.message.wizard',
            'type': 'ir.actions.act_window',
            'res_id': wiz.id,
            'target': 'new'

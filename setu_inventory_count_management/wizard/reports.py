...
class InvCountReport(models.TransientModel):
    ...
    def generate_report(self):
       ...
       select 
        ...
        and count_line.is_discrepancy_found = 't' and count_line.state = 'Approve'
...
class InvUserStatesReport(models.TransientModel):
    ...
    def generate_report(self):
        ...
        action = self.sudo().env.ref('setu_inventory_count_management.inventory_user_states_report_act_window').read()[
            0]
        return action
...
class InventoryCountValuationUpDownReport(models.TransientModel):
    _name = 'inventory.count.valuation.report'
    _inherit = 'setu.inventory.reporting.template'
    ...
    valuation_graph = fields.Selection(
        [('UP', 'UP'),
         ('DOWN', 'DOWN')],
        string="Up/Down"
    )
    ...
    def generate_report(self):
        ...
        if self.end_date:
            end_date = str(self.end_date)  # change_time_zone(local_timezone, str(self.end_date) + ' 23:59:59')
        self._cr.execute('delete from inventory_count_valuation_report;')
        query = f"""select 
                        sml.product_id,
                        sml.lot_id,
        ...
        action = \
            self.sudo().env.ref('setu_inventory_count_management.inventory_count_valuation_report_act_window').read()[0]
        return action
